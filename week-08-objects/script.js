...
function loadContent() {
  ...
  bookArray.sort((a, b) => compare(a.title, b.title));
...
function addNewBook() {
  ...
  const newBook = {title: newBookTitle, pages: newBookPages, author: newBookAuthor};
...
function loadShortestBook(){
  // Assume the first book is shortest
  let shortestBook = bookArray[0];
  ...
  document.getElementById('shortest-book').innerText = shortestBook.title;
}
...
function loadAverage (){
 ...
 for(let i = 0; i < bookArray.length; i++){
  ...
  let average = total / bookArray.length;
  console.log (average);
